...
def plot_cov(en_scores, ma_scores, plot_squares=False):
    ...
    ax.set_xlim(*x_range)
    ax.set_ylim(*y_range)
    ax.set_xlabel('English Points', fontsize=16)
    ax.set_ylabel('Math Points', fontsize=16)
...
def plot_cov_interact(en_scores, ma_scores):
    ...
    @interact(AEnglish=x_range, AMath=y_range,
              BEnglish=x_range, BMath=y_range,
              CEnglish=x_range, CMath=y_range,
              DEnglish=x_range, DMath=y_range)
    def plot(AEnglish=en_scores[0], AMath=ma_scores[0],
             BEnglish=en_scores[1], BMath=ma_scores[1],
             CEnglish=en_scores[2], CMath=ma_scores[2],
             DEnglish=en_scores[3], DMath=ma_scores[3]):
        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(111)
        ax.grid(False)

        names = ['A', 'B', 'C', 'D']
        en_scores = [AEnglish, BEnglish, CEnglish, DEnglish]
        ma_scores = [AMath, BMath, CMath, DMath]
        en_mean = np.mean(en_scores)
        ma_mean = np.mean(ma_scores)
